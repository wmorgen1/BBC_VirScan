...
viruses_totest <- viruses_totest %>% filter(virus != "Human adenovirus 14") %>% 
  ...
  filter(virus != "Human hepatitis A virus genotype IA") # did not converge, not significant in previous iterations

survival_outputs <- data.frame(matrix(nrow = 47, ncol = 87)) 
...
for(i in c(1:47)){
  ...
  curr_model <-  glm(`kid pos` ~ `cc` + `breast_fed` + mage + preterm1 + 
                       ...
                       `Mom RSV positive` + `Mom VZV positive` + `years`
  ...
  survival_outputs$`Mom RSV Estimate`[i] <- curr_model$coefficients[42]
  survival_outputs$`Mom RSV P`[i] <- curr_summary[["coefficients"]][42,4]
  survival_outputs$`Mom VZV Estimate`[i] <- curr_model$coefficients[43]
  survival_outputs$`Mom VZV P`[i] <- curr_summary[["coefficients"]][43,4]
  survival_outputs$`Age Estimate`[i] <- curr_model$coefficients[44]
  survival_outputs$`Age P`[i] <- curr_summary[["coefficients"]][44,4]
}
...
write_rds(survival_outputs, "Data/20250329 - environmental associations with seroconversion.rds")
