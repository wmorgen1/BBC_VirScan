...
phiphfc_3 <- fread("Data/phipseq_0252r_0254r_fiftysixmer_VirscanLar_000_Hits_foldchange_annotated.tsv") %>%
  filter(taxon_species %in% BBC_VARscores_pairs_mat_removed_totest$taxon_species)
...
correlation_data_pair <- correlation_data_pair %>% dplyr::select(-X, -Pearson.R, -peptide.lm.coefficient) %>%
