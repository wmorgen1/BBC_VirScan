...
ggsave("C:/Users/wmorgen1/Documents/R/Larman_Lab/BBC/Figures/20240515 - CMV VARscore dotplot by breastfeeding.jpeg",
       dotplot,
       scale = 1,
       height = 1.5,
       width = 1.5,
...
fisher.test(matrix(data = c(229, 361,
                            21,  163), nrow = 2)) # Mixed vs formula only. OR 4.9, p = 2e-13

fisher.test(matrix(data = c(31, 30,
                            21,  163), nrow = 2)) # Breastfed only vs formula only. OR 7.9, p = 9e-10

fisher.test(matrix(data = c(31, 30,
                            229, 361), nrow = 2)) # Breastfed vs formula only. OR 1.6, p = 0.07
...
timeplot <- ggplot() + 
  geom_point(data = hsv_VAR %>% ungroup() %>% filter(Plasma_type == "child") %>%
               ...
               mutate(breast_fed1 = case_when(
                 ...
                 breast_fed1 == "1" ~ "Mixed",
                 breast_fed1 == "2" ~ "Breastfed"
                 )) %>% mutate(breast_fed1 = as.factor(breast_fed1)) %>%
               mutate(breast_fed1 = fct_relevel(breast_fed1, c("Formula", "Mixed", "Breastfed"))),
...
ggsave("C:/Users/wmorgen1/Documents/R/Larman_Lab/BBC/Figures/20240515 - child CMV VARscore vs time by feeding dotplot.jpeg",
       timeplot,
       scale = 1,
       height = 1.5,
       width = 3,
...
ggsave("C:/Users/wmorgen1/Documents/R/Larman_Lab/BBC/Figures/20240515 - HSV1 VARscore dotplot by breastfeeding.jpeg",
       dotplot,
       scale = 1,
       height = 1.5,
       width = 1.5,
...
fisher.test(matrix(data = c(167, 23,
                            493,  74), nrow = 2)) # Mixed vs formula only. OR 4.9, p = 2e-13

fisher.test(matrix(data = c(167, 23,
                            47,  6), nrow = 2)) # Breastfed only vs formula only. OR 7.9, p = 9e-10

fisher.test(matrix(data = c(23, 167,
                            13, 104), nrow = 2)) # no difference compared to mom-
...
ggsave("C:/Users/wmorgen1/Documents/R/Larman_Lab/BBC/Figures/20240515 - child HSV1 VARscore vs time by feeding dotplot.jpeg",
       timeplot,
       scale = 1,
       height = 1.5,
       width = 3,
