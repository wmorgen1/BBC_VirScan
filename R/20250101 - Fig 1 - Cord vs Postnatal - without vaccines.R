...
BBC_VARscores <- BBC_VARscores %>% filter(sample_id %in% sample_meta$lab_id) %>%
  ...
  filter(!(taxon_species %in% c("Varicella-zoster virus", "Rubella virus",
                                "Rotavirus A", "Rotavirus B", "Rotavirus C", "Rotavirus X",
                                "Mumps virus", "Measles virus", "Influenza A virus",
                                "Influenza B virus", "Influenza C virus", "Human herpesvirus 3",
                                "Hepatitis B virus", "Hepatitis B virus genotype A2 subtype adw2",            
                                "Hepatitis B virus genotype A3", "Hepatitis B virus genotype B2",                         
                                "Hepatitis B virus genotype C subtype adr" ,"Hepatitis B virus genotype D",                          
                                "Hepatitis B virus genotype D subtype ayw", "Hepatitis B virus genotype E",                          
                                "Hepatitis B virus genotype F2", "Hepatitis B virus genotype H subtype adw4",             
...
write.csv(correlation_data, "BBC/Data/20250101 - BBC cord child correlation no vaccines - VARscore.csv")
...
BBC_VARscores_pairs_mat_removed_totest <- BBC_VARscores_pairs_mat_removed %>%
  filter(taxon_species %in% viruses_totest$taxon_species)
...
volcano_data <- read.csv("/BBC/20240424 - BBC cord child volcano.csv")

#spearman volcano
ggplot() + geom_point(data = volcano_data,
                      aes(x = `Spearman rho`,
                          y = `Spearman p`)) +
  theme_light()
...
ggsave("C:/Users/wmorgen1/Documents/R/Larman_Lab/BBC/Figures/20240424 - cord child fisher test volcano.jpeg",
       curr_plot,
       scale = 1,
       height = 2,
       width = 2,
...
phiphfc_1 <- fread("/BBC/phipseq_0232_0241_VirscanLar_000_Hits_foldchange_annotated.tsv") %>%
  filter(taxon_species %in% viruses_totest$taxon_species)
phiphfc_2 <- fread("/BBC/phipseq_0242_0254_VirscanLar_000_Hits_foldchange_annotated.tsv") %>%
  filter(taxon_species %in% viruses_totest$taxon_species)
